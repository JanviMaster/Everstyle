import React from 'react'
import Footer from '../components/Footer'
import './Page.css'

const Aboutus = () => {
  return (
    <>
    <div className='everstyle-main' style={{ padding: '70px' }}>
    <center><p className='head'>ABOUT US</p></center>
    <br/>
      <h2>Who We Are</h2>
      <p>
        At <b>Everstyle</b>, we’re passionate about delivering high-quality products that fit your lifestyle and meet your needs. Since our inception in 2023, we’ve been on a mission to create a seamless and enjoyable shopping experience for our customers.

        Our journey began with a simple idea: to offer a curated selection of products that combine value and quality. Today, we are proud to serve in Indiia and have grown into one of the most trusted e-commerce platforms.
      </p>
      <br/>
    <h2>Our Mission</h2>
    <p>
    Our mission is simple: to provide top-quality products with exceptional customer service. We believe that shopping should be easy, convenient, and affordable, and we are committed to ensuring that every product we sell meets the highest standards of quality.

    We strive to constantly improve and expand our offerings to meet the changing needs of our customers. Whether you’re shopping for shirts, skirts, dresses, jeans etc, our goal is to make your experience smooth and satisfying.
    </p>
    <br/>
    <h2>Our Values</h2>
    <p>
    <b>Customer First:</b> We prioritize customer satisfaction above all else. Your feedback and experience are the most important aspects of our business.<br/>
    <b>Quality & Innovation:</b> We continuously seek out high-quality, innovative products that enhance your lifestyle and provide the best value.<br/>
    <b>Sustainability:</b> We are committed to offering eco-friendly products and packaging solutions that help reduce environmental impact.<br/>
    <b>Integrity:</b> Transparency, trust, and ethical business practices are the foundation of everything we do.
    </p>
    <br/>
    <h2>What We Offer</h2>
    <p>
    At Everstyle, you’ll find a wide range of products, including:</p>
    <ul>
    <li className='li-text'> High-quality products designed to meet your everyday needs.</li>
    <li className='li-text'> Stylish and modern dresses, perfect for any occasion.</li>
    <li className='li-text'> Styles that combines innovation with functionality.</li>
    </ul>
    <p>
    Our team works tirelessly to handpick each product, ensuring that it’s up to our quality standards and that we’re offering the best possible price. We believe in creating value for you in every way.
    </p>
    <br/>
    <h2>Why Shop With Us?</h2>
    <p>
    <b>Fast & Secure Shipping –</b> We deliver across India with a variety of shipping options tailored to your needs.</p>
    <p><b>Customer Support –</b> Our support team is always here to assist you with any queries or concerns, ensuring a hassle-free experience from start to finish.</p>
    <p><b>Easy Returns – </b>If something doesn’t meet your expectations, our simple return policy makes it easy to get a refund or exchange.</p>
    <br/>
    <h2>Our Promise</h2>
    <p>We are committed to maintaining long-term relationships with our customers by offering a convenient shopping experience, top-quality products, and excellent customer service. Evverstyle is here to make your life easier and more enjoyable by bringing you the best of what’s out there.
    </p>
    <br/>
    <p style={{ textAlign: "center" }}>
    Thank you for being a part of our journey!
    </p>
    </div>
    <Footer /> 
</> 
  )
}

export default Aboutus
